...
export class SubmissionsService {
  ...
  private async validateSubmission(answers: Submission[]) {
    ...
    for (const answer of answers) {
       ...
       */
      if (!currentSection || (currentSection.id !== answer.sectionId && !nextQuestionId && nextSectionId === answer.sectionId)) {
        const section = await this.prisma.questionSection.findUnique({ where: { id: answer.sectionId }, include: { previousSection: true, nextSection: true, conditioned: true } });

        // Check if section exists
        if (!section)
          throw new NotFoundException(`Section #${answer.sectionId} does not exist`);

        // Check if the section is the first element of the submission matches with the database
        if (!currentSection && (section.previousSection || section.conditioned.length > 0))
          throw new BadRequestException(`Section #${answer.sectionId} is not the first`);
      ...
      const question = await this.prisma.question.findFirst({ where: { id: answer.questionId, sectionId: answer.sectionId }, include: { nextQuestion: true, conditions: true } });
      if (!question) 
        throw new NotFoundException(`Question #${answer.questionId} on section #${answer.sectionId} does not exist`);
